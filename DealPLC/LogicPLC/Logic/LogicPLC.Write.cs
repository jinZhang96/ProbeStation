    ...
    partial class LogicPLC
    {
        ...
        void ClearPLC(string strReg)
        {
            try
            {
                ...
                ParLogReadWritePLC.P_I.AddInfo_Write(strReg, 0);//添加到读写日志中
        ...
        public void FinishPhoto(string strReg, int intResult)
        {
            ...
            try
            {
                ...
                if (ParSetPLC.P_I.TypePLCProtocol_e == TypePLCProtocol_enum.FullNew)
                {
                    blResult = WriteReg(str[0], 0);//清空拍照和结果寄存器是不连续的
            ...
            try
            {
                    ...
                    || numCo == numData)
                {
                    ...
                    for (int i = 0; i < numData; i++)
                    {
                        ...
                        {
                            string regLog = reg[i];
                            ParLogReadWritePLC.P_I.AddInfo_Write(regLog, value[i]);//记录到写日志
                        }
                        catch (Exception ex)
                        {
                            Log.L_I.WriteError(NameClass, ex);
                    ...
                    for (int i = 0; i < numReg; i++)
                    {
                        ParLogReadWritePLC.P_I.AddInfo_Write(reg[i], value[i]);//拍照结果
                    }
        ...
        public void PCConnectPLC()
        {
            try
            {
                ...
                ParLogReadWritePLC.P_I.AddInfo_Write(ParSetPLC.P_I.regPCConnectPLC, 1);//添加到读写日志中
            }
            catch (Exception ex)
            {
                Log.L_I.WriteError(NameClass, ex);
        ...
        public void PCDisConnectPLC()
        {
            try
            {
                ...
                ParLogReadWritePLC.P_I.AddInfo_Write(ParSetPLC.P_I.regPCConnectPLC, 0);//添加到读写日志中
            }
            catch (Exception ex)
            {
                Log.L_I.WriteError(NameClass, ex);
