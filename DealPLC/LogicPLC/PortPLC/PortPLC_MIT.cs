        ...
        void StartWrapper()
        {
            try
            {
                try
                {
                    ...
                    Process[] pPLC = Process.GetProcessesByName("ActUtlWrapper");//中间通信软件
                ...
                //拷贝DLL到执行文件下方
                string[] path = Directory.GetDirectories("..\\机器视觉控制处理软件\\");
                string pathDllM = "..\\机器视觉控制处理软件\\Interop.ActMsgWrapperLib.dll";
                string pathDllW = "..\\机器视觉控制处理软件\\Interop.ActUtlWrapperLib.dll";
                if (!File.Exists(pathDllM))
                {
                    File.Copy(@"C:\Mod\Interop.ActMsgWrapperLib.dll", pathDllM, true);
                }
                if (!File.Exists(pathDllW))
                {
                    File.Copy(@"C:\Mod\Interop.ActUtlWrapperLib.dll", pathDllW, true);
                }
            }
            catch (Exception ex)
            {
