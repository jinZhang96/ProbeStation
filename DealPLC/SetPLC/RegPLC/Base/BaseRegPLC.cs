    ...
    public class BaseRegPLC : BaseClass
    {
        ...
        public virtual bool ReadIni()
        {
            ...
            try
            {
                ...
                //寄存器个数
                NumRegSet = IniFile.I_I.ReadIniInt("NumRegSet", "NumRegSet", 10, PathRegIni);
                for (int i = 0; i < NumReg; i++)
                {
                    ...
                    //记录到读写日志类中
                    if (nameReg.Replace("\\n", "\n") != ""
                        && !ParLogReadWritePLC.P_I.Ht_RegPLC.Contains(nameReg.Replace("\\n", "\n")))
                    {
                        ParLogReadWritePLC.P_I.Ht_RegPLC.Add(nameReg.Replace("\\n", "\n"), inst);
                    }
