...
namespace ModulePackage
{
    ...
    public class CursoryLocation : ModuleBase
    {
        ...
        /// <returns></returns>
        public static bool SendRobotPickPos(Point2D pt2Pixel, Point4D pt4Dst, Point4D pt4Adj, string cmd, BaseParCalibRobot parCalibrate,out Point4D pResult)
        {
            pResult = new Point4D();
            try
            {
                ...
                ShowState(string.Format("发送给机器人坐标:X:{0},Y:{1},R:{2}", 
                    result.DblValue1.ToString(ReservDigits), result.DblValue2.ToString(ReservDigits), result.DblValue4.ToString(ReservDigits)));
            ...
            BaseParCalibRobot parCalibrate, out Point4D result)
        {
            ...
            try
            {
                ...
                result = new Point4D(pt2Map.DblValue1 + pt4Adj.DblValue1, pt2Map.DblValue2 + pt4Adj.DblValue2, pt4Dst.DblValue3 + pt4Adj.DblValue3, pt4Dst.DblValue4 + pt4Adj.DblValue4);
            }
            catch (Exception ex)
            {
                Log.L_I.WriteError(ClassName, ex);
