    ...
    public class BackLightLocation : ModuleBase
    {
            ...
            double botAngle, double displayAngle, double blAngle, int id, bool ifRecord, out Point2D deltaDst)
        {
            ...
            try
            {
                ...
                ShowState(string.Format("精定位偏差X:{0}，Y{1}", deltaDst.DblValue1.ToString("f3"), deltaDst.DblValue2.ToString(ReservDigits)));
                ...
                ShowState(string.Format("发送放片坐标:X:{0},Y:{1},R:{2}", pt4Dst.DblValue1.ToString(ReservDigits), pt4Dst.DblValue2.ToString(ReservDigits), pt4Dst.DblValue4.ToString(ReservDigits)));
                if (ifRecord)
                    RecordPreciseData(id, "第二次精定位结果：X-->" + deltaDst.DblValue1.ToString(ReservDigits) + " Y-->" + deltaDst.DblValue2.ToString(ReservDigits) + " R-->" + r.ToString(ReservDigits));
                return true;
            }
            catch (Exception ex)
            {
            ...
            double srcAngle, double dstAngle, double botAngle, double displayAngle, double blAngle)
        {
            try
            {
                ...
                ShowState(string.Format("一次拍照计算的偏差结果：X:{0}/Y:{1}", delta.DblValue1.ToString(ReservDigits), delta.DblValue2.ToString(ReservDigits)));
            }
            catch (Exception ex)
            {
                Log.L_I.WriteError(ClassName, ex);
