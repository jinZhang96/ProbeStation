...
namespace Main_EX
{
    partial class WinInitMain
    {
        #region 定义
        //int 
        int NumErrorForChangeModel = 0;

        bool BlFromCopy = false;//是否拷贝了参数
        ...
        public bool NewModel()
        {
            try
            {
                ...
                ShowState("正在新建文件");
                //ShowState_EN("Creating New Recipe");
                ...
                {
                    ShowState("新文件路径更改成功");
                    //ShowState_EN("Path Of New Recipe is Successful");
                }
        ...
        public bool ChangeModel()
        {
            try
            {
                NumErrorForChangeModel = 0;

                g_BlFinishBasePar = false;

                ShowState("正在换型,请稍等......");
                //ShowState_EN("Changing  Recipe");

                //将配置文件路径写入文件
                if (!ParManageConfigPar.P_I.WriteIniPathConfigPar())
                {
                    NumErrorForChangeModel++;
                    ShowAlarm("新配置文件路径更改失败");
                    //ShowAlarm_EN("Path of New Recipe is Failed");
                }
                else
                {
                    ShowState("新配置文件路径更改成功");
                    //ShowAlarm_EN("Path of New Recipe is Successful");
                }
                ...
                if (!ParConfigPar.P_I.ReadIniConfigPar())
                {
                    ...
                    //ShowAlarm_EN("Par of Product is Successful");
                }
                else
                {
                    ShowState("读取新的产品参数");
                ...
                if (!ParDelFolder.P_I.ReadIniPar())//删除文件设定
                {
                   ...
                   // ShowAlarm_EN("File of Monitor is Successful");
                }
                else
                {
                ...
                //ShowAlarm_EN("Reading Par of Camera");
            ...
            {
                ShowAlarm("换型失败");
                //ShowAlarm_EN("Change Recipe is Failed");
                Log.L_I.WriteError(NameClass, ex);
                return false;
            }
            finally
        ...
        void ChangeXmlComprehensive1()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowAlarm("相机1综合设置参数读取失败");
                    if (ParComprehensive1.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive1.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive2()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowAlarm("相机2综合设置参数读取失败");
                    if (ParComprehensive2.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive2.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive3()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowWinError_Invoke("相机3综合设置参数读取失败");
                    if (ParComprehensive3.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive3.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive4()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowWinError_Invoke("相机4综合设置参数读取失败");
                    if (ParComprehensive4.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive4.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive5()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowWinError_Invoke("相机5综合设置参数读取失败");
                    if (ParComprehensive5.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive5.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive6()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowWinError_Invoke("相机6综合设置参数读取失败");
                    if (ParComprehensive6.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive6.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive7()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowWinError_Invoke("相机7综合设置参数读取失败");
                    if (ParComprehensive7.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive7.P_I.Error);
                    }
                }
                else
                {
        ...
        void ChangeXmlComprehensive8()
        {
            try
            {
                ...
                {
                    NumErrorForChangeModel++;
                    ShowWinError_Invoke("相机8综合设置参数读取失败");
                    if (ParComprehensive8.P_I.Error != "")
                    {
                        BlFromCopy = true;
                        ShowWinError_Invoke(ParComprehensive8.P_I.Error);
                    }
                }
                else
                {
            ...
            try
            {
                    ...
                    && g_BlFinishComprehensive)
                {
                    if (NumErrorForChangeModel > 0)
                    {
                        ...
                        {
                            if (BlFromCopy)//如果恢复过参数
                            {
                                WinMsgBox.ShowMsgBox("Change Recipe is Failed:By The Info,Check Par from CopyFile.If The Par is Right,Then Change Recipe is Successed!!", true, 3000);
                            }
                            else
                            {
                                WinMsgBox.ShowMsgBox("Change Recipe is Failed!", true, 3000);
                            }
                        }
                        else
                        {
                            if (BlFromCopy)//如果恢复过参数
                            {
                                WinMsgBox.ShowMsgBox("换型失败:需要按照提示,检查一下从备份恢复的文件!文件如果正确，则换型成功", false);
                            }
                            else
                            {
                    ...
                    {
                        ShowState("换型成功");
                        //ShowState_EN("Change Recipe is Successed");
                        if (ParLanguage.P_I.Language_e == Language_enum.EN)
                        {
                            WinMsgBox.ShowMsgBox("Change Recipe is Successed!", true, 3000);
                        }
                        else
                        {
