    ...
    public partial class BaseDealComprehensiveResult_Main
    {
        ...
        private void GetStdInsertPos()
        {
            try
            {
                ...
                CSTLocation.StdInsert_L = CSTLocation.CreateKeelStdPos(
                    stdPos, ModelParams.confCSTCol, ModelParams.confKeelInterval, ModelParams.confCol1Interval, ModelParams.DirInsert);
            }
            catch (Exception ex)
            {
                Log.L_I.WriteError(NameClass, ex);
        ...
        private bool CalcDevHeight()
        {
            try
            {
                ...
                CSTLocation.LayerSpacing_L = CSTLocation.CalcLayerSpacing(
                    ModelParams.confCSTCol, ModelParams.confCSTRow, ModelParams.confLayerSpacing, CSTLocation.TopDev_L, CSTLocation.BottomDev_L);
                ...
                LogicPLC.L_I.WriteRegData3((int)DataRegister3.InsertComZ1 + CSTLocation.CurrentCstNo - 1, CSTLocation.HeightDev_L.Count, CSTLocation.HeightDev_L.ToArray());
                LogicPLC.L_I.WriteRegData3((int)DataRegister3.KeelSpacing1 + (CSTLocation.CurrentCstNo - 1) * 6, CSTLocation.LayerSpacing_L.Count, CSTLocation.LayerSpacing_L.ToArray());
        ...
        public void VerifyCSTData()
        {
            try
            {
                ...
                blError = VerifyInsertDev() || VerifyKeelSpacing();// || //VerifyHeightDev() ||
                   // VerifyKeelHeightDev() || VerifyLayerSpacing();

                if (blError)
                {
                    ShowAlarm("通知PLC卡塞不符合规格");
            ...
            try
            {
                ...
                for (int i = 0; i < CSTLocation.KeelHeightDev_L.Count; ++i)
                {
                    ...
                    ShowState(string.Format("卡塞第{0}列上龙骨左右龙骨高度相差{1}mm,阈值{2}",
                        i + 1, CSTLocation.KeelHeightDev_L[i].DblValue1.ToString(ReservDigits), ModelParams.CSTThread_KeelHeight));
                    //校验部分
