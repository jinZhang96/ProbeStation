    ...
    public partial class BaseDealComprehensiveResult_Main
    {
        ...
        public StateComprehensive_enum BinLocation(Point2D std)
        {
            if (blValid2_BinCam1 && blValid2_BinCam2)
            {
                ...
                double r = GetCurAngle(ModelParams.confDisMark, AMP, pt2MarkArray[(int)PtType_Mono.AutoMark1], pt2MarkArray[(int)PtType_Mono.AutoMark2]);
                if (!ModuleBase.GetPtAfterRotate(pt2MarkArray[(int)PtType_Mono.AutoMark1], -r, strCamera5RC, ParComprehensive5.P_I, out Point2D ptAfterR))
                {
                    ShowState("旋转中心计算失败");
        ...
        {
            return Calibration(pt2MarkArray[(int)PtType_Mono.CalibMark1], pt2MarkArray[(int)PtType_Mono.CalibMark2], ModelParams.confDisMark, ParCalibWorld.V_I[g_NoCamera], 0);
        }
        ...
        public bool CalibStd()
        {
            try
            {
                if (blValid3_BinCam1 && blValid3_BinCam2)
                {
                    ...
                    double r1 = ModuleBase.GetCurAngleByX(ModelParams.confDisMark, AMP, pt2MarkArray[(int)PtType_Mono.CalibMark1], pt2MarkArray[(int)PtType_Mono.CalibMark2]);
                    double r2 = ModuleBase.GetCurAngleByX(ModelParams.confDisMark, AMP, pt2MarkArray[(int)PtType_Mono.R180Mark1], pt2MarkArray[(int)PtType_Mono.R180Mark2]);
                    if (Math.Abs(r1 - r2) > 0.2)
                    {
                        ShowAlarm(string.Format("相机{0}两次标定角度差别过大", g_NoCamera));
                    }

                    return RecordStdValue(strCamera5Match1, pt2MarkArray[(int)PtType_Mono.CalibMark1], pt2MarkArray[(int)PtType_Mono.R180Mark1]);
                }
            }
            catch (Exception ex)
            {
        ...
        /// <param name="r"></param>
        /// <returns></returns>
        public bool VerifyRotateCenter(Point2D pt2MarkSrc1, Point2D pt2MarkSrc2, Point2D pt2MarkDst1, Point2D pt2Mark2Dst2, double r, double thread)
        {
            //求取旋转前和旋转后的玻璃角度
